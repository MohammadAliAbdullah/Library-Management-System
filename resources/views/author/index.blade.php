...
<div id="addDepartmentFormContainer"> 
      ...
      <div class="form-group">
        <div class="form-row">
            ...
            {!! Form::text('title','',['id'=>'department','class'=>'form-control','placeholder'=>'Author Name']) !!}
        </div>
         </div>
        </div>           
...
<button class="btn btn-primary" id="NewDepartmentBtn">ADD NEW AUTHOR</button>
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="">
                ...
                <th width="60%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Department: activate to sort column ascending">Author Name</th>
            ...
            {{-- <td><img src="../{{$author->picture }}" width="50px" height="50px"/></td> --}}
                {{-- <td>{{($loop->index + 1)}}</td>             --}}
                <td>{{$loop->iteration}}</td>            
                <td class="sorting_1">{{$department->id}}</td>
                <td class="hidden-xs">{{$department->authorname}}</td>
                <td>
...
$("#contentContainer").on('click','#editBtn', function(){
            ...
            // console.log($dep);
            // var url = "{{URL::to('/')}}"; // Url path pubilc till 
            $info_url = url + '/admin/author/'+$departmentid+'/edit';
            // alert($info_url);
            //  exit;
            $.get($info_url,{
			// alert(4);	
...
$("#addDepBtn").click(function(){
    if($(this).val() == 'Update'){
        $.ajax({
            // alert(5);
            // exit;
                url:url+'/admin/author/'+$("#department_id").val(),
                method: "PUT",
                type: "PUT",
                data: {
                     authorname: $('#department').val() 
...
$("#contentContainer").on('click','#deleteBtn', function(){
           ...
           // console.log($departmentid);
            $info_url = url + '/admin/author/'+$departmentid;
...
$("#searchBtn").click(function(){
 ...
 populateSearchData(d);
    ...
    //  <td>'+.($loop->index + 1)+'</td>
            $h +=  '<tr role="row" class="odd"><td>1</td><td class="sorting_1">'+data.id+'</td><td class="hidden-xs">'+data.authorname+'</td><td><button type="submit" id="editBtn" rid='+data.id+' class="btn btn-warning btn-margin">Update</button><button type="submit" id="deleteBtn" rid='+data.id+' class="btn btn-danger btn-margin">Delete</button></td></tr>';
...
// populateForm function only for edit

        function populateForm(data){
            $("#department_id").val(data.id);
            $("#department").val(data.authorname);
